...
def filter_cells(datasets: dict, included_labels: list, min_genes: int=200, min_cells: int=200):
    for label in included_labels:
...
def extract_common_hvgs(datasets: dict, included_labels: list, n_top_genes: int = 1000):
...
def add_metadata(datasets: dict, filepath: str, included_labels: list, is_float=True):
    ...
    #md_sel[AD_status_lbl] = md_sel[AD_status_lbl].method(v_yes)
    md_sel[AD_status_lbl].replace(to_replace='no', value=v_no, inplace=True)
    #md_sel[AD_status_lbl] = md_sel[AD_status_lbl].method(v_no)
...
def save_files(datasets: dict, filepath: str, included_labels: list):
    ...
    for label in included_labels:
        ...
        p = os.path.join(filepath, 'processed_data_modified/')
        f = label + '.h5ad'
        to = os.path.join(p,f)
...
def pipeline(to_include: list):
    ...
    filter_cells(datasets, included_labels, min_cells=200, min_genes=200)
